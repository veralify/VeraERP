...
/// Where this month's income goes, as charts.
///
/// Embedded in the roadmap rather than standing on its own: the roadmap answers
/// "what does this route cost", these answer "where is the money going now",
/// and the two are the same question at different ranges. Content only — the
/// scroll view and padding belong to whoever embeds it.
///
/// Takes the summary rather than building its own. Half a dozen properties here
/// read it, and rebuilding it on each read ran the payoff engine every time.
    ...
    var body: some View {
        if hasData {
            VStack(spacing: 18) {
                ...
                allocationCard
                // No balance-over-time chart here: the roadmap's own
                // "What you still owe" plots the same figures directly above
                // this, from the same plan.
                if !debts.isEmpty { debtMixCard }
            }
        } else {
            EmptyStateView(
                icon: "chart.pie",
