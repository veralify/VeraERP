    ...
    private var balanceCard: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack(alignment: .top) {
                ...
                Spacer(minLength: 8)
                Mascot(net: net, hasEntries: !entries.isEmpty)
            }
...
/// A character that reacts to the month. Deliberately never says "well done"
/// when the balance is negative — a finance app cheering on a deficit is the
/// same mistake as colouring it lime.
struct Mascot: View {
    let net: Decimal
    let hasEntries: Bool

    private var message: LocalizedStringKey {
        guard hasEntries else { return "Add an entry and I'll keep score." }
        if net > 0 { return "Nice — you're up this month." }
        if net < 0 { return "You're spending more than you take in." }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 8) {
            Text(message)
                ...
                .padding(10)
                .background(Theme.surfaceElevated, in: .rect(cornerRadius: 12))
